...
def plot_ratio_by_ranking(
    ...
    def compute_hit_ratios(
        ...
        for i in range(0, len(sorted_peptides), step_size):
            ...
            area_under_curve += hit_ratio * (
                min(i + step_size, len(sorted_peptides)) - i
            )
    ...
    if plot_theoretical_maximums:
        dummy_optimal_mdm2_ranking = mdm2_hits + (
            [""] * (len(peptides_test) - len(mdm2_hits))
        )
        ...
        dummy_optimal_ca5_ranking = ca5_hits + (
            [""] * (len(peptides_test) - len(ca5_hits))
        )
        ...
        plt.plot(
            1 - theoretical_best_mdm2_ratios[:, 1],
            theoretical_best_mdm2_ratios[:, 0],
            ":",
            label="Theoretical max MDM2",
            color='r'
        )
        plt.plot(
    ...
    plt.fill_between(
        ca5_hit_ratios[:, 1], y1=ca5_hit_ratios[:, 0], y2=0, color="b", alpha=0.3
    ) 
    plt.plot(
        1 - mdm2_hit_ratios[:, 1],
        mdm2_hit_ratios[:, 0],
    ...
    plt.fill_between(
        1 - mdm2_hit_ratios[:, 1], y1=mdm2_hit_ratios[:, 0], y2=0, color="r", alpha=0.3
    ) 

    plt.legend()
    plt.title(title)
    plt.ylabel("Hit Rate Ratio")
    plt.xlabel("Peptide Proportion Ranked from 0.0 (12ca5) to 1.0 (MDM2)")
